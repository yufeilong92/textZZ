...
public class DownloadListViewAdapter extends BaseAdapter {
    ...
    public DownloadListViewAdapter(DownVideoDb lists, Context context, ListView lv_download) {
...
//        mDao = DbHelperDownAssist.getInstance();
    ...
    public void downloadAll() {
        ...
        for (int i = 0; i < lists.size(); i++) {
            ...
            if (progress == 100)
                finishKey.add(PolyvDownloaderManager.getKey(downloadInfo.getVid(), Integer.parseInt(downloadInfo.getBitRate())));
        }
    ...
    public void deleteTask(int position) {
        DownVideoVo downloadInfo = lists.remove(position);
        //移除任务
        PolyvDownloader downloader = PolyvDownloaderManager.clearPolyvDownload(downloadInfo.getVid(), Integer.parseInt(downloadInfo.getBitRate()));
        //删除文件
        downloader.deleteVideo();
        //移除数据库的下载信息
        mDao.delectItem(mDownVideo,downloadInfo);
...
//                        iv_start.setImageResource(R.drawable.polyv_btn_download);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        DownVideoVo downloadInfo = lists.get(position);
        String vid = downloadInfo.getVid();
        int bitrate = Integer.parseInt(downloadInfo.getBitRate());
        long percent = downloadInfo.getPercent();
        long total = downloadInfo.getTotal();
    ...
    private class MyDownloadListener implements IPolyvDownloaderProgressListener {
        ...
        MyDownloadListener(Context context, ListView lv_download, ViewHolder viewHolder, DownVideoVo downloadInfo, int position) {
...
//                viewHolder.get().iv_start.setImageResource(R.drawable.polyv_btn_dlpause);
    ...
    private static void showPauseSpeeView(DownVideoVo downloadInfo, TextView tv_speed) {
        long percent = downloadInfo.getPercent();
        long total = downloadInfo.getTotal();
        int progress = 0;
        if (total != 0)
    ...
    private class ViewHolder {
        ...
        public void setDownloadListener(PolyvDownloader downloader, final DownVideoVo downloadInfo, final int position) {
    ...
    private class DownloadOnClickListener implements View.OnClickListener {
        ...
        public DownloadOnClickListener(DownVideoVo downloadInfo, ImageView iv_start, TextView tv_status, TextView tv_speed) {
            this.downloadInfo = downloadInfo;
        ...
        public void onClick(View v) {
            ...
            if (tv_status.getText().equals(DOWNLOADED)) {
                Intent intent = NetBookMyInfomActivity.newIntent(context, NetBookMyInfomActivity.PlayMode.portrait, vid, bitrate, true, true);
//                 在线视频和下载的视频播放的时候只显示播放器窗口，用该参数来控制
//                intent.putExtra(NetBookMyInfomActivity.IS_VLMS_ONLINE, false);
